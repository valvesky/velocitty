...
test "copy stream trims trailing spaces" {
    const gpa = std.testing.allocator;
    var screen = try Term.Screen.init(gpa, 8, 2);
    defer screen.deinit();
    ...
    const text = try copyAlloc(gpa, &screen, .{
        .on = true,
        .a = .{ .col = 0, .row = 0 },
        .b = .{ .col = 7, .row = 1 },
    });
...
test "word and line select" {
    const gpa = std.testing.allocator;
    var screen = try Term.Screen.init(gpa, 8, 1);
    defer screen.deinit();
    const Runs = @import("runs.zig");
    var runs: std.ArrayList(Runs.Run) = .empty;
    defer runs.deinit(gpa);
    ...
    try Runs.split(gpa, src, &runs);
    screen.feed(runs.items, src);
    const w = wordAt(&screen, 1, 0);
    try std.testing.expectEqual(@as(u16, 0), w.a.col);
    try std.testing.expectEqual(@as(u16, 1), w.b.col);
    const text = try copyAlloc(gpa, &screen, w);
    defer gpa.free(text);
    try std.testing.expectEqualStrings("ab", text);
    const line = try copyAlloc(gpa, &screen, lineAt(&screen, 0));
    defer gpa.free(line);
    try std.testing.expectEqualStrings("ab cd", line);
}

test "contains is inclusive stream" {
    var sel: State = .{};
    sel.begin(2, 0);
    sel.extend(1, 1);
    try std.testing.expect(sel.contains(2, 0, 4));
    try std.testing.expect(sel.contains(0, 1, 4));
    try std.testing.expect(sel.contains(1, 1, 4));
    try std.testing.expect(!sel.contains(1, 0, 4));
    try std.testing.expect(!sel.contains(2, 1, 4));
    try std.testing.expect(sel.coversRow(0, 4));
    try std.testing.expect(sel.overlapsRows(0, 1, 4));
}
